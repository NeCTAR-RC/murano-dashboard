...
urlpatterns = [
    urls.re_path(r'^environments/$',
                 views.IndexView.as_view(),
                 name='index'),
    urls.re_path(r'^create_environment$',
                 views.CreateEnvironmentView.as_view(),
                 name='create_environment'),
    urls.re_path(ENVIRONMENT_ID + r'/services$',
                 views.EnvironmentDetails.as_view(),
                 name='services'),
    urls.re_path(ENVIRONMENT_ID + r'/services/get_d3_data$',
                 views.JSONView.as_view(),
                 name='d3_data'),
    urls.re_path(ENVIRONMENT_ID + r'/(?P<service_id>[^/]+)/$',
                 views.DetailServiceView.as_view(),
                 name='service_details'),
    urls.re_path(ENVIRONMENT_ID + r'/start_action/(?P<action_id>[^/]+)/$',
                 views.StartActionView.as_view(),
                 name='start_action'),
    urls.re_path(ENVIRONMENT_ID +
                 r'/actions/(?P<task_id>[^/]+)(?:/(?P<optional>[^/]+))?/$',
                 views.ActionResultView.as_view(), name='action_result'),
    urls.re_path(r'^(?P<instance_id>[^/]+)/$',
                 inst_view.DetailView.as_view(), name='detail'),
    urls.re_path(r'^deployment_history$',
                 views.DeploymentHistoryView.as_view(),
                 name='deployment_history'),
    urls.re_path(ENVIRONMENT_ID + r'/deployments/(?P<deployment_id>[^/]+)$',
